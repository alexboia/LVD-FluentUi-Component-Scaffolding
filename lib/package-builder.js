...
class ComponentPackageBuilder {
	...
	_creatPackageModelReader() {
		return new ComponentPackageModelReader(this._options, 
			this._logger);
	}
	...
	_determineDestinationPath() {
		return this._options.createRoot 
			? path.join(this._baseDestinationPath, this._packageModelData.packageNameLower)
			: this._baseDestinationPath;
	}
