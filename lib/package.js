...
class ComponentPackage {
	constructor(destinationPath, template, logger) {
		if (!destinationPath) {
	...
	create(onReady) {
		const me = this;
		me.logger.debug('Collecting package information from user input...');
		me._readPackageModel(function() {
			if (me._hasFilledInPackageModel()) {
				me._createPackage(onReady);
			} else {
				onReady(false);
			}
		});
	...
	_readPackageModel(onReady) {
		...
		input.question('Package name = ', function(packageName) {
			if (!packageName) {
				me.logger.reportProgressError('Package name cannot be empty!');
				input.close();
				return;
			}
	...
	_createPackage(onReady) {
		...
		this.logger.debug('Building package contents...');
		const packageContents = this._processTemplate(this.template);
		this.logger.debug('Done building package contents.');

		const destinationPath = this._determineDestinationPath();
		this.logger.debug(`Using package destination path: ${destinationPath}. Will ensure it exists...`);
		this._ensureDestinationPathExists(destinationPath);

		this.logger.debug('Writing package contents on disk...');
		this._writePackageContents(packageContents, destinationPath);
		this.logger.debug('Done writing package contents.');

		this.logger.debug('Attempting to install package dependencies...');
		this._installPackageDependencies(destinationPath, function(success) {
			if (!success) {
				me.logger.reportProgressWarning('There may have been an issue with installing dependencies. Please consult the logs for more details.');
			} else {
				me.logger.debug('Successfully installed package dependencies.');
			}
			onReady(true);
		});
	}
