...
class ComponentPackage {
	...
	_createPackage(onReady) {
		...
		this.logger.debug('Creating package structure on disk...');
		this._writePackageContents(packageContents, 
			destinationPath);
		...
		this._installPackageDependencies(destinationPath, function(success) {
			if (!success) {
				me.logger.reportProgressWarning('There may have been an issue with running npm install. Please consult the logs for more details.');
			}
			onReady(true);
		});
	}
